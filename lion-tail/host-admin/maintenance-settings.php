...
if (isset($_POST['post'])) {
  ...
  if (!empty($setting)) {
    if (query_sql("UPDATE $codex SET `status`='" . $setting . "' WHERE `id`=1")) {
    ...
    <section id="main-content">
      <section class="wrapper">
        ...
        <?php } else { ?>
          <div class="row">
            <div id="go" class=" col-lg-12">
              <div id="go" class="alert alert-warning" style="text-align: center; color:#000;"> Set System on maintenance mood (Yes => Maintenance, No => Working mood) <i id="remove" class="fa fa-remove pull-right"></i></div>
            </div>
          </div>
        <?php } ?>
        <div class="row">
            ...
            <form action="" method="post" enctype="multipart/form-data">
              <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                <tbody>
                  <tr>
                    <td>
                      <label>Maintenance Setting:</label>

                      <select class="form-control" name="set" id="set">
                        <option selected="selected" value="">Set Mood</option>
                        <?php $set = $cal->selectFrmDB($codex, 'status', 'id', 1); ?>
                  ...
                  <tr>
                    <td align="right"><input class="btn btn-primary" type="submit" name="post" value="Set Mood" /></td>
                  </tr>
                </tbody>
              </table>
            </form>
