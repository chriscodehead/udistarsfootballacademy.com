...
$total_pages_sql = "SELECT COUNT(*) FROM $gallery_tb";
  ...
  <div class="our-news-div-main py-5">
   <div class="container">
    ...
    <div class="row row-cols-1 row-cols-md-2 row-cols-lg-3 gy-5 gx-md-5 gy-lg-0 gx-lg-5 mt-0">
     ...
     <?php $sql = query_sql("SELECT * FROM $gallery_tb ORDER BY id DESC LIMIT $offset, $no_of_records_per_page");
     if (mysqli_num_rows($sql) > 0) {
      $c = 0;
      while ($row = mysqli_fetch_assoc($sql)) { ?>
       <div class="col mb-3">
        <a class="comon-blogs1">
         <figure>
          <img src="<?php print 'photo/' . $row['image']; ?>" alt="<?php print $row['caption']; ?>" />
         </figure>
         <div class="news-items-del">
          <h5><?php print $row['caption']; ?></h5>
         </div>
        </a>
       </div>
      <?php $c++;
